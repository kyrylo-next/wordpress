...
if ( ! class_exists( 'YITH_WCPB_Frontend' ) ) {
	...
	class YITH_WCPB_Frontend {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_cart_item_class', array( $this, 'table_item_class_bundle' ), 10, 3 );
		...
		public function table_item_class_bundle( $classname, $cart_item, $cart_item_key = '' ) {
			...
			if ( $is_bundled_item ) {
				$classname .= ' yith-wcpb-child-of-bundle-table-item ';
			} elseif ( $is_bundle ) {
				$classname .= ' yith-wcpb-bundle-table-item ';
			}

			if ( 'woocommerce_cart_item_class' === current_action() && $cart_item_key && $is_bundled_item ) {
				...
				if ( $idx && isset( $cart_item_keys[ $idx + 1 ] ) ) {
					...
					if ( ! isset( $next_cart_item['bundled_by'] ) ) {
						$classname .= ' yith-wcpb-child-of-bundle-table-item--last ';
					}
				}
			}
